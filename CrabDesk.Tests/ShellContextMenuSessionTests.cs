...
public sealed class ShellContextMenuSessionTests
{
    ...
    [Fact]
    public async Task NativeShellMenuCanOpenAndCloseForMultipleItemsInSameFolder()
    {
        var root = Path.Combine(Path.GetTempPath(), $"CrabDeskShellMenu-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        var paths = new[]
        ...
        foreach (var path in paths)
        {
            await File.WriteAllTextAsync(path, "native shell menu multi-select lifetime test");
        }
        ...
        var thread = new Thread(() =>
        {
            try
            {
                ...
                form.Show();
                using var session = ShellContextMenuSession.TryCreate(paths, form.Handle);
                if (session is null)
                {
                    completion.SetResult(false);
                    return;
